...
import { Filter, Search, RotateCcw, ShieldCheck, Play, Award, Check } from 'lucide-react';
...
export default function CatalogPage({ initialCategory, searchQuery, onSelectAnimal }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8 pb-28">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-theme-border pb-6">
        <div>
          ...
          <p className="text-xs sm:text-sm text-theme-muted mt-1">
            Menampilkan {animals.length} hewan ternak sehat bersertifikasi resmi
          </p>
        </div>
      ...
      {isFilterDrawerOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm lg:hidden flex justify-end animate-in fade-in duration-150">
          <div className="w-full max-w-xs bg-theme-card h-full p-6 space-y-5 overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between border-b border-theme-border pb-3">
              ...
              <button
                ...
                className="p-1 rounded-lg text-theme-muted hover:text-theme-text font-bold text-base"
              >
                ✕
              </button>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  className="w-full bg-theme-bg border border-theme-border rounded-xl pl-9 pr-3 py-2 text-xs text-theme-text"
                />
                <Search className="w-4 h-4 text-theme-muted absolute left-3 top-2.5" />
              </div>
            </div>
            ...
            <div className="space-y-1.5 pt-2 border-t border-theme-border">
              <label className="text-xs font-bold text-theme-text uppercase tracking-wider block">Kategori</label>
              <select
                value={category}
                onChange={(e) => setCategory(e.target.value)}
                className="w-full bg-theme-bg border border-theme-border rounded-xl px-3 py-2 text-xs text-theme-text"
              >
                ...
                <option value="SAPI">Sapi Qurban & Pedaging</option>
                <option value="DOMBA">Domba Garut</option>
                <option value="KAMBING">Kambing Etawa</option>
                <option value="KERBAU">Kerbau Rawa</option>
              </select>
            </div>
            ...
            <div className="space-y-1.5 pt-2 border-t border-theme-border">
              ...
              >
                <option value="">Semua Rentang Harga</option>
                <option value="under_10">Di bawah Rp 10 Juta</option>
                <option value="10_30">Rp 10 Juta - Rp 30 Juta</option>
                <option value="30_50">Rp 30 Juta - Rp 50 Juta</option>
                <option value="above_50">Di atas Rp 50 Juta</option>
              </select>
            </div>
            ...
            <div className="space-y-1.5 pt-2 border-t border-theme-border">
              ...
              >
                ...
                <option value="50_100">50 kg - 100 kg</option>
                <option value="100_500">100 kg - 500 kg</option>
                <option value="above_500">Di atas 500 kg (Jumbo)</option>
              </select>
            </div>
            ...
            <div className="pt-4 flex items-center gap-2">
              <button
                onClick={handleResetFilters}
                className="w-1/2 py-2.5 rounded-xl border border-theme-border text-xs font-bold text-theme-text hover:bg-theme-bg"
              >
                Reset
              </button>
              <button
                onClick={() => setIsFilterDrawerOpen(false)}
                className="w-1/2 py-2.5 rounded-xl bg-theme-primary text-white text-xs font-bold shadow-md"
              >
                Terapkan
              </button>
            </div>
